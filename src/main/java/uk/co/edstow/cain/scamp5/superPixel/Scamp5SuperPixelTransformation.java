...
public abstract class Scamp5SuperPixelTransformation<G extends BankedKernel3DGoal<G>> implements BankedTransformation {
    ...
    public static class Null<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        @Override
        public OutputCode code(List<BRegister> uppers, List<BRegister> lowers, List<BRegister> trash) {
    ...
    abstract static class SimpleScamp5SuperPixelTransformation<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        abstract OutputCode code(BRegister upper, List<BRegister> lowers);
...
//            assert lowers.size() == inputCount();
            ...
            code.addOutput(this.config.outputFormatter.comment(String.format("/*SP Res(%s)*/ ", uppers)));
            ...
            for (int bank = 0; bank < config.banks; bank++) {
                ...
                if (!regs.isEmpty()){
                    ...
                    config.selectBank(code, bank, config.maskReg, scratch);
                    for (BRegister reg: regs){
                        String sreg = reg.name;
                        code.addOutput(config.outputFormatter.NOT(config.scratchRegisters.get(0), sreg));
                        code.addOutput(config.outputFormatter.NOR(sreg, config.scratchRegisters.get(0), config.maskReg));
                    }
                }
            }
    ...
    public static class Negate<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        public OutputCode code(List<BRegister> uppers, List<BRegister> lowers, List<BRegister> trash) {
            ...
            OutputCode code = new OutputCode();
            code.addOutput(config.outputFormatter.comment(String.format("/*SP Neg(%s, %s)*/ ", uppers.get(0), lowers.get(0))));
            String outputReg = uppers.get(0).name;
            String inputAReg = lowers.get(0).name;

            for (int i = 1; i <= config.getBits(bank); i++) {
                code.addOutput(config.outputFormatter.comment(String.format("\n/* Bit %d */", i)));
                // Copy in Carry in from correct PE
                config.setDirLessSignificantAndSelectBit(code, bank, i, config.maskReg, Collections.emptyList());
                if (i == 1) { // If least significant bit set carry in directly based on if we're doing a sub or add
                    code.addOutput(config.outputFormatter.SET(config.maskedReg));
                } else {// for the other bits read in from less significant bit
                    code.addOutput(config.outputFormatter.DNEWS0(config.maskedReg, config.scratchRegisters.get(0)));
                }
                // maskedReg := Carry in bit

                // Use dir registers as scratch registers
                code.addOutput(config.outputFormatter.NOT(config.northReg, inputAReg));   // vn(!a) = !a
                code.addOutput(config.outputFormatter.NOT(config.eastReg, config.maskedReg));   // ve(!c_in) = !vM(c_in)
                code.addOutput(config.outputFormatter.NOR(config.scratchRegisters.get(0), inputAReg, config.eastReg)); // s0(c_out) = !(!!a + ve(!c_in))
                code.addOutput(config.outputFormatter.NOR(config.southReg, config.northReg, config.maskedReg)); // vs(!(!a+c_in)) = !(vn(!a) + vM(c_in))
                code.addOutput(config.outputFormatter.NOR(config.maskedReg, config.scratchRegisters.get(0), config.southReg)); // vM(sum) = !(s0(c_out) + vs(!(!a+c_in)))
                // scratch[0] := Carry out bit
                code.addOutput(config.outputFormatter.MOV(outputReg, config.maskedReg));
    ...
    public static abstract class AddSub_2<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        protected static OutputCode longAddCode(Scamp5SuperPixelConfig config, List<BRegister> uppers, List<BRegister> lowers) {
            assert uppers.size() == 1;
            assert lowers.size() == 2;
            assert uppers.get(0).bank == lowers.get(0).bank && uppers.get(0).bank == lowers.get(1).bank;
            int bank = uppers.get(0).bank;
            OutputCode code = new OutputCode();
            code.addOutput(config.outputFormatter.comment(String.format("/*SP add(%s, %s, %s)*/ ", uppers.get(0), lowers.get(0), lowers.get(1))));
            String outputReg = uppers.get(0).name;
            String inputAReg = lowers.get(0).name;
            ...
            for (int i = 0; i < config.getBits(bank); i++) {
                code.addOutput(config.outputFormatter.comment(String.format("\n/* Bit %d */", i)));
                // Copy in Carry in from correct PE
                config.setDirLessSignificant(code, bank);
                // the least significant bit will have zeros for n,e,s,w so this should clear maskReg to 0 for that PE
                code.addOutput(config.outputFormatter.DNEWS0(config.maskReg, config.scratchRegisters.get(0)));
                ...
                code.addOutput(config.outputFormatter.NOR(config.northReg, inputAReg, inputBReg));           //vn(1) = !(a+b)
                code.addOutput(config.outputFormatter.NOR(config.eastReg, inputAReg, config.northReg));      //ve(2) = !(a+vn(1))
                code.addOutput(config.outputFormatter.NOR(config.southReg, inputBReg, config.northReg));     //vs(3) = !(b+vn(1))
                code.addOutput(config.outputFormatter.NOR(config.westReg, config.eastReg, config.southReg)); //vw(4) = !(ve(2)+vs(3))
                code.addOutput(config.outputFormatter.NOR(config.eastReg, config.maskReg, config.westReg)); //ve(5) = !(vm(C)+vw(4))
                code.addOutput(config.outputFormatter.NOR(config.southReg, config.westReg, config.eastReg)); //vs(6) = !(vw(4)+ve(5))
                code.addOutput(config.outputFormatter.NOR(config.westReg, config.maskReg, config.eastReg)); //vw(7) = !(vm(C)+ve(5))
                code.addOutput(config.outputFormatter.NOR(scratch.get(0), config.eastReg, config.northReg)); //v0(C) = !(ve(5)+vn(1))
                // scratch[0] := Carry out bit

                // Multiplex to only store sum if we're in the correct bank.
                code.addOutput(config.outputFormatter.SET(config.maskReg));
                code.addOutput(config.outputFormatter.MOV(config.maskedReg, outputReg));
                config.selectBank(code, uppers.get(0).bank, config.maskReg, Arrays.asList(config.northReg, config.eastReg));
                code.addOutput(config.outputFormatter.NOR(config.maskedReg, config.southReg, config.westReg)); //vm(S) = !(vs(6)+vw(7))
                code.addOutput(config.outputFormatter.MOV(outputReg, config.maskedReg));
        ...
        protected static OutputCode shortCode(Scamp5SuperPixelConfig config, BRegister upper, BRegister lowerA, BRegister lowerB, boolean sub) {
            assert upper.bank == lowerA.bank && upper.bank == lowerB.bank;
            assert !upper.equals(lowerA);
            assert !upper.equals(lowerB);
            int bank = upper.bank;
            OutputCode code = new OutputCode();
            code.addOutput(config.outputFormatter.comment(String.format("/*SP %s(%s, %s, %s)*/ ", sub?"Sub":"Add", upper, lowerA, lowerB)));
            ...
            code.addOutput(config.outputFormatter.SET(config.maskReg));
            code.addOutput(config.outputFormatter.MOV(config.maskedReg, outputReg));
            config.selectBank(code, bank, config.maskReg, Arrays.asList(config.northReg, config.eastReg, config.southReg, config.westReg));

            if (sub) { // invert b if we want to subtract
                code.addOutput(config.outputFormatter.NOT(config.westReg, inputBReg));
            ...
            code.addOutput(config.outputFormatter.NOR(config.eastReg, inputAReg, config.northReg));      //ve(2) = !(a+vn(1))
            code.addOutput(config.outputFormatter.NOR(config.southReg, inputBReg, config.northReg));     //vs(3) = !(b+vn(1))
            code.addOutput(config.outputFormatter.NOR(config.maskedReg, config.eastReg, config.southReg)); //vM(4) = !(ve(2)+vs(3))
            code.addOutput(config.outputFormatter.MOV(outputReg, config.maskedReg));                      //vO(4) = vM(4)
            // v(4) is in maskedReg
            ...
            for (int i = 1; i <= config.getBits(bank); i++) {
                ...
                if (i == 1) { // If least significant bit set carry in directly based on if we're doing a sub or add
                    if (sub) {
                        ...
                        code.addOutput(config.outputFormatter.CLR(config.maskedReg));
                    }
                } else {// for the other bits read in from less significant bit
                    code.addOutput(config.outputFormatter.DNEWS0(config.maskedReg, config.scratchRegisters.get(0)));
                }
                ...
                // Use dir registers as scratch registers
                if (sub) { // invert b if we want to subtract
                    code.addOutput(config.outputFormatter.NOT(config.westReg, lowerB.name));   // vw(!b) = !b (for subtraction)
                }
                code.addOutput(config.outputFormatter.NOR(config.northReg, inputAReg, inputBReg));           //vn(1) = !(a+b)
                code.addOutput(config.outputFormatter.NOR(config.eastReg, config.maskedReg, outputReg)); //ve(5) = !(vM(C)+vO(4))
                code.addOutput(config.outputFormatter.NOR(config.southReg, outputReg, config.eastReg)); //vs(6) = !(vO(4)+ve(5))
                code.addOutput(config.outputFormatter.NOR(config.westReg, config.maskedReg, config.eastReg)); //vw(7) = !(vM(C)+ve(5))
                code.addOutput(config.outputFormatter.NOR(config.scratchRegisters.get(0), config.eastReg, config.northReg)); //v0(C) = !(ve(5)+vn(1))
                // scratch[0] := Carry out bit
    ...
    public static class AddSelf<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        public OutputCode code(List<BRegister> uppers, List<BRegister> lowers, List<BRegister> trash) {
            ...
            int bank = uppers.get(0).bank;
            OutputCode code = new OutputCode();
            code.addOutput(config.outputFormatter.comment(String.format("/*SP addSelf(%s, %s)*/ ", uppers.get(0), lowers.get(0))));
            String outputReg = uppers.get(0).name;
            ...
            code.addOutput(config.outputFormatter.SET(config.maskReg));
            code.addOutput(config.outputFormatter.MOV(config.maskedReg, outputReg));
            config.selectBank(code, uppers.get(0).bank, config.maskReg, scratch);
            code.addOutput(config.outputFormatter.DNEWS0(config.maskedReg, inputReg));
            code.addOutput(config.outputFormatter.MOV(outputReg, config.maskedReg));
    ...
    public static class Div<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        public OutputCode code(List<BRegister> uppers, List<BRegister> lowers, List<BRegister> trash) {
            ...
            int bank = uppers.get(0).bank;
            OutputCode code = new OutputCode();
            code.addOutput(config.outputFormatter.comment(String.format("/*SP Div(%s, %s)*/ ", uppers.get(0), lowers.get(0))));
            String outputReg = uppers.get(0).name;
            String inputReg = lowers.get(0).name;
            List<String> scratch = config.scratchRegisters;

            // Copy in Carry in from correct PE
            config.setDirMoreSignificant(code, bank);
            ...
            code.addOutput(config.outputFormatter.OR(scratch.get(0), config.northReg, config.eastReg, config.southReg, config.westReg));
            code.addOutput(config.outputFormatter.NOT(config.northReg, config.maskReg));
            code.addOutput(config.outputFormatter.NOR(config.maskReg, scratch.get(0), config.northReg));
            code.addOutput(config.outputFormatter.MOV(config.maskedReg, inputReg));
    ...
    public static class Movxb<G extends BankedKernel3DGoal<G>> extends Scamp5SuperPixelTransformation<G> {
        ...
        public OutputCode code(List<BRegister> uppers, List<BRegister> lowers, List<BRegister> trash) {
            ...
            assert lowers.size() == 1;
            OutputCode code = new OutputCode();
            code.addOutput(config.outputFormatter.comment(String.format("/*SP movb(%s, %s)*/ ", uppers.get(0), lowers.get(0))));
            ...
            if(xPETranslation != 0) {
                ...
                // set move direction
                code.addOutput(config.outputFormatter.CLR(config.northReg, config.eastReg, config.southReg, config.westReg));
                code.addOutput(config.outputFormatter.SET(dirReg, config.maskReg));//also set maskReg so we can use maskedReg
                for (int i = 0; i < Math.abs(xPETranslation); i++) {
                    if(i%2 == 0) { // ensure we never do a Mov where dst and src are the same!
                        code.addOutput(config.outputFormatter.DNEWS0(scratchReg, currentReg));
                        currentReg = scratchReg;
                    }else{
                        code.addOutput(config.outputFormatter.DNEWS0(config.maskedReg, currentReg));
            ...
            if (xPETranslation == 0 && yPETranslation == 0){ // catch case when translation is (0,0)
                code.addOutput(config.outputFormatter.SET(config.maskReg));// set maskReg so we can use maskedReg
            }
            List<String> scratch = new ArrayList<>(config.scratchRegisters);
            scratch.add(config.northReg);
            scratch.add(config.eastReg);
            scratch.add(config.southReg);
            scratch.add(config.westReg);
            if(currentReg.equals(config.maskedReg)){
                // the moved result is in maskedReg
                config.selectBank(code, ubank, config.selectReg, scratch); // invert selectBank into maskReg
                code.addOutput(config.outputFormatter.NOT(config.maskReg, config.selectReg));
                code.addOutput(config.outputFormatter.MOV(config.maskedReg, output)); // copy original output value if this PE is Not in bank
            } else {
                // the moved result is in currentReg (and currentReg isn't maskedReg)
                code.addOutput(config.outputFormatter.MOV(config.maskedReg, output)); // copy original output value
                config.selectBank(code, ubank, config.maskReg, scratch); // selectBank into maskReg
                code.addOutput(config.outputFormatter.MOV(config.maskedReg, currentReg)); // copy moved value if PE is in Bank
            }

            code.addOutput(config.outputFormatter.MOV(output, config.maskedReg)); // copy result into outputReg
            code.addOutput(config.outputFormatter.newLine());
            return code;
        }
